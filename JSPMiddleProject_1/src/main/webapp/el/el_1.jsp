...
<%--
		...
		EL : 표현식 (브라우저에 데이터 출력) <%= %>
		     ${출력할 데이터}
		JSTL : 태그형 라이브러리 => 자바의 제어문 , 변수선언 , 화면 이동 
							    날짜 변환 , 숫자 변환
	    ...
	    => 출력시
	       연산처리 => 조건 , 출력(EL)
	          산술연산자 : + , - , * , / , %
	                   ...
	                   == 문자열 결합( =+)
	               ** ${"10"+10}
	                 ---- 자동으로 Inter.parseInt("10") => 정수변경
	                 ---- 결과값 20
	               ** $ {null + 10}
	                     ---- 자동으로 0으로 대체
	               ** EL에서 처리는 문자 / 날짜도 사용된다
	               				 ---------
	          비교연산자 => 결과값은 true / flase
	              		   문자 / 날짜도 비교가 가능
	             == (eq) , != (ne) , < (gt) , > (lt) ,
	             ${10==10} ${10 eq 10}
	...
	=> 제어문 (JSTL) => JSP => MVC 사용시에는 자바를 사용하지 않는다
	  ...
	  *** = fmt : 날짜 변환 , 숫자 변환 => uri=""
	        	  ------   --------
	        	  		   | DecimalFormat => 천단위,
	        	  | SimpleDataForamat => yyyy-MM-dd
	        	  ---------------------------------
	        	  | 많이 사용하지 않는다 : 오라클에서 변경이 가능
	        	  					  TO_CHAR
	        	  <fmt:formatDate>
	        	  <fmt:formatNumber>  			 
      *** = fn : String클래스의 메소드 호출 => uri=> "jakarta.tags.futter"
                   length(), substring()..
...
<body>
  ...
  %>
  <h3>&lt;c:forEach&gt; 사용</h3>
  <c:forEach var="i" begi="1" end="10" step="1">
     ${i }&nbsp;
  </c:forEach>
</body>
</html>