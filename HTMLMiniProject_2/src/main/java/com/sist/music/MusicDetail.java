...
public class MusicDetail extends HttpServlet {
	...
	out.println("<td width=60%>"+vo.getState()+", 순위변동 수: "+vo.getHit()+"</td>");
	out.println("</tr>");
	...
	for(ReplyVO rvo:list)
	{
		...
		out.println("<input type=hidden name=rno value="+mno+">");
	...
	out.println("<h3>인기뮤직</h3>");
	List<MusicVO> fList=dao.musicHitTop10();
	...
	for(MusicVO fvo:fList)
	{
		...
		out.println("<td>"+fvo.getTitle()+"</td>");
